...
const animate = function () {
    ...
    cloths.forEach((cloth) => {
        ...
        if (intersects.length > 0) {
            ...
            cloth.uniforms.touchUv = { value: intersects[0].uv }; // exactly where the mouse is on the surface of the object
            cloth.uniforms.touchStrengthAim = { value: 1 };
        } else {
            cloth.uniforms.touchStrengthAim = { value: 0 };
        }

        // the following chunk of code serves the purpose of avoiding the abrupt interruption of the depression
        // caused by the hovering of the mouse on the cloth when you leave the object area
        // so when hovering, the depression is caused, and when leaving the shape with the mouse
        // the depression "fades" away gradually
